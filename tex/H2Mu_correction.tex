...
Instead, the correction is derived from the generated muon information, smeared with some funtional forms to match the experimental resolution.
...
and other long-lived particles that produce a vertex displaced from the colliding region in the transverse plane at the scale of a millemeter or above.
...
The study shown in this section reports the finding that this false displacement in prompt tracks
has a strong geometrical correlation with the mis-measurement of the track momemtum.
...
In this setup, a few geometric relationships can be found between different variables, listed as follows:
...
Both the PV and the BS are reasonable representation of the colliding position and are useful in different cases.
...
The $(\pt^{reco}-\pt^{gen})/ (\pt^{gen})^{2}$ vs d0 correlation is found to be different in different |\eta| regions and data-taking periods: 
...
\begin{figure*}[!htb]
      ...
      \caption{Plots credit to Pierluigi Bortignon.
               Plots of the \mmm shape of the 2018 \ggH simulation sample, comparing different muon corretion methods.
